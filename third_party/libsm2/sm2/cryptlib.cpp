...
int crypt_enc_init(cipher_context *cctx, uint8 *ck, int ckl, uint8 *iv, int alg)
{
	...
	case ALG_AES:
		...
		if(ckl<AES_KEY_LENGTH)
			return 0;
		AES_set_encrypt_key(ck, ckl*8, (AES_KEY *)cctx->enc_ctx);
		if(iv)
	...
	case ALG_SM4:
		...
		if(ckl<SM4_KEY_LENGTH)
			return 0;
		sm4_setkey_enc((sm4_context *)cctx->enc_ctx, ck);
		if(iv)
			memcpy(cctx->iv, iv, cctx->block_size);
		break;
	default:
...
int crypt_dec_init(cipher_context *cctx, uint8 *ck, int ckl, uint8 *iv, int alg)
{
	...
	case ALG_AES:
		...
		if( alg == ALG_AES_CTR )
			...
			AES_set_decrypt_key(ck, ckl*8, (AES_KEY *)cctx->enc_ctx);
		if(iv)
			memcpy(cctx->iv, iv, cctx->block_size);
		break;
	case ALG_SM4:
		...
		if( alg == ALG_SM4_CTR )
			...
			sm4_setkey_dec((sm4_context *)cctx->enc_ctx, ck);
		if(iv)
			memcpy(cctx->iv, iv, cctx->block_size);
		break;
	default:
...
int crypt_enc_block(cipher_context *cctx, uint8 *in, uint8 *out, int len)
{
	...
	case ALG_AES_CTR:
	{
		...
		for(j=0; j<len; j+=cctx->block_size){
			AES_encrypt(in+j, out+j, (AES_KEY *)cctx->enc_ctx);
		}
		return 1;
	}
	case ALG_AES_CBC:
	{
		...
		for(j=0; j<len; j+=cctx->block_size){
			for(i=0; i<cctx->block_size; i++)
				temp[i]=in[j+i]^piv[i];
			AES_encrypt(temp, out+j, (AES_KEY *)cctx->enc_ctx);
			piv=out+j;
		}
	...
	case ALG_SM4_ECB:
	case ALG_SM4_CTR:
		sm4_crypt_ecb((sm4_context *)cctx->enc_ctx, SM4_ENCRYPT, len, in, out);
		return 1;
	case ALG_SM4_CBC:
		sm4_crypt_cbc((sm4_context *)cctx->enc_ctx, SM4_ENCRYPT, len, cctx->iv, in, out);
		return 1;
	}
...
int crypt_dec_block(cipher_context *cctx, uint8 *in, uint8 *out, int len)
{
	...
	case ALG_AES_CTR:
	{
		...
		for(j=0; j<len; j+=cctx->block_size){
			AES_decrypt(in+j, out+j, (AES_KEY *)cctx->enc_ctx);
		}
		return 1;
	}
	case ALG_AES_CBC:
	{
		...
		for(j=0; j<len; j+=cctx->block_size){
			...
			AES_decrypt(temp, out+j, (AES_KEY *)cctx->enc_ctx);
			for(i=0; i<cctx->block_size; i++){
				out[j+i]^=cctx->iv[i];
			}
	...
	case ALG_SM4_ECB:
	case ALG_SM4_CTR:
		sm4_crypt_ecb((sm4_context *)cctx->enc_ctx, SM4_DECRYPT, len, in, out);
		return 1;
		break;
	case ALG_SM4_CBC:
		sm4_crypt_cbc((sm4_context *)cctx->enc_ctx, SM4_DECRYPT, len, cctx->iv, in, out);
		return 1;
		break;
	}
